...
WORKDIR     /go/src/github.com/golang-ddd-postgresql-auth
ADD         .   /go/src/github.com/golang-ddd-postgresql-auth
COPY        scripts/dep .
RUN         chmod +x dep; ./dep
...
COPY        --from=builder /go/src/github.com/golang-ddd-postgresql-auth/bin/golang-ddd-postgresql-auth golang-ddd-postgresql-auth
ENTRYPOINT  [ "/golang-ddd-postgresql-auth" ]
